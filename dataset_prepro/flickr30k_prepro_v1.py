...
def create_parser():
    ...
    parser.add_argument(
        '--output_prefix', type=str, default='flickr30k')
    parser.add_argument(
...
if __name__ == '__main__':
    ...
    with open(json_path, 'r') as f:
        dataset_f30k = json.load(f)
    
    ### Tokenise captions ###
    
    tokenised_f30k = prepro.tokenise(dataset_f30k,
                                     image_id_key='imgid',
    ...
    if build_vocab:
        wtoi, itow, cw = prepro.build_vocab(tokenised_f30k,
    ...
    tokenised_f30k = prepro.tokenised_word_to_txt_V1(tokenised_f30k,
                                                     args.caption_len_thres,
                                                     include_restval=False)
    
    print('\nINFO: Example captions:')
    for j in range(5):
        print(tokenised_f30k['train'][j])
    print('\n')
    ...
    if args.output_prefix is not None:
        ...
        for split in tokenised_f30k.keys():
            filename = '{}_{}_{}.txt'.format(args.output_prefix, split, suffix)
            with open(pjoin(out_path, filename), 'w') as f:
                f.write('\r\n'.join(tokenised_f30k[split]))
        
        # Assert no overlaps between sets
        train_set = set([s.split(',')[0] for s in tokenised_f30k['train']])
        valid_set = set([s.split(',')[0] for s in tokenised_f30k['valid']])
        test_set = set([s.split(',')[0] for s in tokenised_f30k['test']])
