...
def get_ngrams(refs_words):
    ...
    time.sleep(0.1)
    _ngram_words = compute_doc_freq(create_crefs(refs_words))
    print('\nINFO: Computing term frequency: indices (SKIPPED)')
    # time.sleep(0.1)
    # _ngram_idxs = compute_doc_freq(create_crefs(refs_idxs))
    _ngram_idxs = None
    return _ngram_words, _ngram_idxs, len(refs_words)
...
if __name__ == '__main__':
    ...
    for d in data:
        if d[0] not in data_dict:
            data_dict[d[0]] = []
        # <EOS> is added to sentences in the original code
        data_dict[d[0]].append(d[1].replace('<GO> ', ''))
        # TODO: maybe remove <EOS> as well?
